...
function VideoPlayer({ src, className, controls = true }) {
...
function MediaCarousel({ media, onOpenLightbox }) {
  ...
  const handleTouchMove = (e) => {
    ...
    if (trackRef.current) {
      ...
      const pxOffset = -(current * containerWidth) + deltaX;
      trackRef.current.style.transform = `translateX(${pxOffset}px)`;
    }
  };
      ...
      >
        ...
        >
          {media.map((item, i) => (
            <div
              key={item.id}
              className="w-full flex-shrink-0 flex items-center justify-center bg-gray-100 cursor-pointer"
              style={{ aspectRatio: "16/9" }}
              onClick={() => {
                if (Math.abs(touchDeltaRef.current) < 5) onOpenLightbox(i);
              }}
            >
        ...
        {media.length > 1 && current > 0 && (
          <button
            ...
            className="absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 bg-white/90 hover:bg-white rounded-full shadow-md flex items-center justify-center cursor-pointer transition-all opacity-0 group-hover:opacity-100 z-10 hover:scale-105"
            style={{ opacity: 1 }}
            aria-label="Previous"
          >
        ...
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-2">
          {media.map((m, i) => (
            <button
              ...
              className={`rounded-sm overflow-hidden border-2 transition-all cursor-pointer ${
                i === current
                  ? "border-white w-12 h-12 opacity-100"
...
export default function MediaGallery({ media }) {
  ...
  // Single media — show it directly without carousel overhead
  if (media.length === 1) {
    const item = media[0];
    return (
      <>
        <div
          className="mt-3 rounded-xl overflow-hidden bg-gray-100 cursor-pointer"
